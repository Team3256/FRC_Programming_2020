...
public class RightDriveTrenchTenBallAutoMode extends AutoModeBase {
    ...
    protected void routine() throws AutoModeEndedException {
        ...
        runAction(new ParallelAction(Arrays.asList(new PurePursuitAction(2), new StartIntakeAction(), new FeederIndexAction())));
        runAction(new WaitAction(0.5));
        runAction(new ParallelAction(Arrays.asList(new PurePursuitAction(3), new StartIntakeAction(), new SeriesAction(Arrays.asList(new HoodZeroAction(1.0), new ShootAction())))));
        runAction(new StopIntakeAction());
        Flywheel.getInstance().setReadyToShoot(true);
        runAction(new ShootAction());
